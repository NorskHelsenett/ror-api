...
func GetByFilter(ctx context.Context, filter *apicontracts.Filter) (*apicontracts.PaginatedResult[*apicontracts.Cluster], error) {
	ctx, span := rortracer.StartSpan(ctx, "clustersservice.GetByFilter")
	defer span.End()

	_, span1 := rortracer.StartSpan(ctx, "getMongodb")
	defer span1.End()

	span1.End()

	_, span2 := rortracer.StartSpan(ctx, "mongoclusters.GetByFilter")
	defer span2.End()
	...
	_, span4 := rortracer.StartSpan(ctx, "fillClusterObject")
	defer span4.End()
...
func CreateOrUpdate(ctx context.Context, input *apicontracts.Cluster, clusterId string) error {
	ctx, span := rortracer.StartSpan(ctx, "Service: clustersservice.CreateOrUpdate")
	defer span.End()
	_, span1 := rortracer.StartSpan(ctx, "Check if cluster exists")
	defer span1.End()
	...
	if existing != nil {
		_, span1 = rortracer.StartSpan(ctx, "Run service update")
		defer span1.End()
		...
		_, span1 = rortracer.StartSpan(ctx, "Run service create")
		defer span1.End()
...
func Update(ctx context.Context, input *apicontracts.Cluster, existing *apicontracts.Cluster) error {
	ctx, span := rortracer.StartSpan(ctx, "Service: clustersservice.Update")
	defer span.End()
	if existing == nil {
	...
	_, span1 := rortracer.StartSpan(ctx, "Run repository update")
	defer span1.End()
...
func GetKubeconfig(ctx context.Context, clusterId string, credentials apicontracts.KubeconfigCredentials) (string, error) {
	ctx, span := rortracer.StartSpan(ctx, "clustersservice.GetKubeconfig")
	defer span.End()
